 ...
 * 3. 생성된 회원 객체 배열에 담아서 출력하시오 
 * 
 * split된 배열에 들어있는 값들로 회원 객체
 */

public class MemberMain {

	public static void main(String[] args) {
		// 기본 데이터를 가공하여 Member 객체로 생성하여 담을 객체 배열 선어
		// Member type 배열 members
		
		// Members 객체를 생성해서 담을 수 있게 딤
		Member[] members = new Member[98]; // Member type 객체 생성

		// 문자열 배열에 들어있는 값으로 객체 생성해서 members에 저장해주는 메소드 호출
		initData(members); // 문자열 배열에 들어있는 값으로 객체 생성해서 members에 저장해주느 메소드 호출

		// 1. 전체 회원 리스트 출력 메소드 만들기
		printMemberList(members);
		
		// [문제1] 20대 후반 여성의 총 방문횟수 구해서 반환해주는 메소드 만들기
		int count = getVisitCount(members);
		System.out.println(count);
		
		// [문제2] 40대 이상의 총 인원 수를 구해주는 메소드 만들기
	...
	private static void initData(Member[] members) {
		...
		// 이 데이터를 split로 잘라서 member 객체를 생성해서 Member[] 배열에 저장하라
		...
		for (int i = 0; i < member.length; i++) {
			String[] arr = member[i].split(","); // 콤머로 구분해서 자름
			//arr[0] = "봉기", arr[1]="20180419", arr[2] = "20181002"
			 int numVisit = Integer.parseInt(arr[3]); //numVisit -> 문자열을 숫자로 변경
	...
	private static void printMemberList(Member[] members) {
		for (int i = 0; i < members.length; i++) {
			System.out.println(members[i].getName()+"\t"+members[i].getJoinDate()+"\t"+members[i].getLastVisitDate()+"\t"+members[i].getNumVisit()+"\t"+members[i].getGender()+"\t"+members[i].getAge());
		}
	}
